...
| `report_family.py` | Prepare full-text journal/direction run bundles by default; deterministic reports with `--metadata-only`; stat reports unchanged | `python scripts/report_family.py --mode journal --journal JOURNAL` |
| `prepare_direction_review.py` | Prepare a direction-level literature review bundle with local full text and web supplementation | `python scripts/prepare_direction_review.py --direction ExampleDirection` |
